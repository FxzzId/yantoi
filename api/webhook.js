...
module.exports = async (req, res) => {
  ...
  if (url === '/api/public-stats' && method === 'GET') {
    ...
    return res.json({ success: true, soldMap, maintenance: data.maintenance || false, totalProducts: (data.products || []).length, totalOrders: orders.length });
  }
  ...
  if (url === '/api/create-cart-order' && method === 'POST') {
    ...
    for (const item of items) {
      ...
      if (!product || product.stock < item.quantity) { allStock = false; break; }
